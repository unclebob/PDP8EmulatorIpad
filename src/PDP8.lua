...
function PDP8:draw()
    -- background(129, 131, 130, 55)
    setContext(self.img)
    -- background(15, 13, 13, 255)
...
function addNewRack()
    ...
    -- print(pdp8.racks[pdp8.currentRackNumber])
...
function RackControls:init(x, y, width)
    ...
    self.loadButton = MomentaryButton(x+(width/3)-self.prevButton.width, y, "Load", loadData)
    ...
    --self.img = image(self.width, HEIGHT)
end
function RackControls:draw()
    ...
    -- 分别绘制左下角的 Prev, Next, Load, Save 按钮
...
function Register:draw()
    ...
    fill(0)
    stroke(0)
    local textx = self.x-w/2
    local texty = self.y+h/2+20-h/2
    --rect(textx-6,texty,w+12,h)
    rect((self.x + Register:widthOfBits()) + Register.spacing,self.y + Bit.height/2 - h/2,w,h)
    setupText()
    local w,h = textSize(self.name)
    text(self.name,
    (self.x + Register:widthOfBits()) + Register.spacing,self.y + Bit.height/2 - h/2)
end
