...
function drawVersion() 
    ...
    fill(255)
    textMode(CORNER)
    text(VERSION, 0, 5+BOTTOM)
end

PDP8 = class()

function setup()
    if HEIGHT>800 then -- bigger ipads, leave room for keyboard banner.
        BOTTOM=50
    else
        BOTTOM=0
    end
...
function PDP8:setupFrame()     
    self.controlPanel = ControlPanel(self.rightFrame,80+BOTTOM)
    self:setupTTY()    
    self:setupPunch()
    self:setupReader()
    self.rimPanel = RimPanel(self.midFrame,80+BOTTOM)        
    self:setupRacks()
    self.testButton = MomentaryButton(self.midFrame, 10+BOTTOM, "Test", testAll)
    self.statsPanel = StatsPanel(WIDTH-StatsPanel.width-30, 10+BOTTOM)    
    self.quitButton = MomentaryButton(self.rightFrame+50, 10+BOTTOM, "QuitX2", quitProgram)
    self.quitTime = 0
end
...
function PDP8:setupTTY()
    local ttyBottom = 90+self.controlPanel.height+BOTTOM
    self.tty = Teletype(self.rightFrame, ttyBottom, self.controlPanel.width, HEIGHT-ttyBottom-10)
    self.junkTtyButton =  MomentaryButton(self.rightFrame+250, 10+BOTTOM, "Junk TTY", junkTty)
    self.ttySpeed = Button(self.midFrame+50, 10+BOTTOM, "TTY FAST", setTtySpeed)
    self.ttyFast = 0    
    self.saveListingButton = MomentaryButton(self.rightFrame+150, 10+BOTTOM, "Save TTY", saveListing)
end
...
function PDP8:setupReader()
    self.tapeReader = TapeReader(self.midFrame, 340+BOTTOM)
...
function PDP8:setupRacks()
    self.currentRack = Rack(0,100+BOTTOM,self.midFrame-self.leftFrame)
    self.rackControls = RackControls(0,40+BOTTOM,Shelf.width*2)
...
function ControlPanel:checkDevs()
    local ttyDelay =  TTYDELAY
    local punchDelay = PUNCHDELAY
    local readerDelay = READERDELAY + self.autoReaderDelay
...
function StatsPanel:draw()
    local msg=string.format("%6d fps\n%6d ips\n%6d ipf",pdp8.framesPerSecond ,pdp8.instructionsPerSecond, cyclesPerFrame)
    font("Courier")
    fontSize(15)
